 ...
 * @author Chris Chabot
 */
class buzzFileStorage extends buzzStorage {
  private $path;
  ...
  private function createLock($storageFile) {
    ...
    if (! is_dir($storageDir)) {
      if (! @mkdir($storageDir, 0755, true)) {
        ...
        if (! is_dir($storageDir)) {
          throw new buzzStorageException("Could not create storage directory: $storageDir");
        }
      }
    }
  ...
  public function set($key, $value) {
    ...
    if (! @file_put_contents($storageFile, $data)) {
      $this->removeLock($storageFile);
      throw new buzzStorageException("Could not store data in the file");
    }
    $this->removeLock($storageFile);
  }
  ...
  public function delete($key) {
    ...
    if (! @unlink($file)) {
      throw new buzzStorageException("Storage file could not be deleted");
    }
  }
}
